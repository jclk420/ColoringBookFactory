        ...
        for number, image_path in enumerate(images, start=1):
            ...
            try:
                ...
                with Image.open(image_path) as image:
                    ...
                    f.write(f"Mode: {mode}\n")
                    if height <= width:
                        message = f"{image_path.name}: image is not portrait ({width}x{height})."
                        warnings.append(message)
                        f.write("WARNING: " + message + "\n")
                    else:
                        f.write("OK: Portrait orientation\n")
                    if width < min_width or height < min_height:
                        message = (f"{image_path.name}: source resolution is low "
                                   f"({width}x{height}); recommended minimum is "
                    ...
                    if mode not in {"RGB", "L"}:
                        message = f"{image_path.name}: color mode is {mode}; it will be normalized to RGB."
                        warnings.append(message)
                        f.write("WARNING: " + message + "\n")
                    else:
                        f.write("OK: Color mode\n")
                    if "transparency" in image.info or "A" in image.getbands():
                        message = f"{image_path.name}: transparency/alpha channel detected; background will be normalized."
                        warnings.append(message)
                        f.write("WARNING: " + message + "\n")
                digest = sha256(image_path.read_bytes()).hexdigest()
                if digest in seen_hashes:
                    message = f"{image_path.name}: duplicate artwork detected; same file content as {seen_hashes[digest]}."
                ...
                message = f"{image_path.name}: cannot be processed: {error}"
                errors.append(message)
                f.write("ERROR: " + message + "\n")
            f.write("\n")
        f.write("=" * 60 + "\n")
...
def build_book(project):
    ...
    print()
    print("=" * 60)
    print("COLORING BOOK FACTORY v2.1")
    print("=" * 60)
    print()
