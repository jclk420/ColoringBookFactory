...
def build_book(project):
    ...
    qc_warnings, qc_errors = quality_check(
        project,
        settings,
        images
    )
    ...
    page_files, page_errors, page_warnings = (
        page_preflight(
            project,
            settings
        )
...
if qc_errors > 0:
    all_errors.append(
        f"Quality control found {qc_errors} error(s)."
    )

if qc_warnings > 0:
    all_warnings.append(
        f"Quality control found {qc_warnings} warning(s)."
    )
