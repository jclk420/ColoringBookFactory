...
# ============================================================
# v7.0 PLATFORM ENGINE
# ============================================================

PLATFORM_ENGINE_VERSION = "1.0"
...
DEFAULT_PLATFORM_PROFILES = {
    "KDP": {
        ...
        "output_dir": "KDP",
        "source": "master_pdf",
        "files": ["interior_pdf", "cover_pdf", "metadata", "checklist"],
        "notes": "Print package. KDP-specific validation remains authoritative."
    },
    "Gumroad": {
        "name": "Gumroad",
        "type": "digital",
        ...
        "output_dir": "GUMROAD",
        "source": "master_pdf",
        "files": ["digital_pdf", "cover_preview", "preview_sheet", "metadata", "product_description"],
        "notes": "Digital delivery package; does not alter the master PDF."
    },
    "Digital PDF": {
        "name": "Universal Digital PDF",
        "type": "digital",
        "enabled": True,
        "output_dir": "DIGITAL",
        "source": "master_pdf",
        "files": ["digital_pdf", "cover_preview", "preview_sheet", "metadata"],
    ...
    "Etsy": {
        ...
        "source": "master_pdf",
        "files": ["digital_pdf", "cover_preview", "preview_sheet", "metadata", "product_description"],
        "notes": "Enable when this platform is part of the publishing plan."
    },
    "Shopify": {
        "name": "Shopify Digital",
        "type": "digital",
        "enabled": False,
        "output_dir": "SHOPIFY",
        "source": "master_pdf",
...
def load_platform_profiles():
    ...
    if not path.exists():
        save_json(path, {"version": PLATFORM_ENGINE_VERSION, "profiles": DEFAULT_PLATFORM_PROFILES})
        return dict(DEFAULT_PLATFORM_PROFILES)
    try:
        data = load_json(path)
        profiles = data.get("profiles", {}) if isinstance(data, dict) else {}
        merged = dict(DEFAULT_PLATFORM_PROFILES)
...
def find_master_pdf(project):
    candidates = [
        project / "PDF" / f"{kdp_safe_filename(load_json(project / 'project.json').get('title', project.name))}.pdf",
        project / "PDF" / "final.pdf",
    ]
    candidates.extend(sorted((project / "PDF").glob("*.pdf")) if (project / "PDF").exists() else [])
    for candidate in candidates:
        if candidate.exists():
            return candidate
    finals = sorted((project / "FINAL").glob("*.pdf")) if (project / "FINAL").exists() else []
...
def find_existing_book_pdf(project):
    ...
    # Prefer an interior/master/final PDF and avoid generated platform copies.
    found = [p for p in found if PLATFORM_DIRNAME not in p.parts]
    if not found:
        return None
...
def ensure_master_book(project, source_pdf=None):
    """Create a platform-neutral MASTER package without modifying existing production artifacts."""
    settings = load_project_settings_safe(project)
    master = platform_master_dir(project)
    pdf = source_pdf or find_master_pdf(project) or find_existing_book_pdf(project)
    if pdf is None:
    ...
    if pdf.resolve() != master_pdf.resolve():
    ...
    for candidate in [project / "COVER", project / "KDP_PACKAGE"]:
        if candidate.exists():
            covers = sorted(candidate.glob("*.pdf")) + sorted(candidate.glob("*.png")) + sorted(candidate.glob("*.jpg"))
            if covers:
                source_cover = covers[0]
                break
    if source_cover:
        shutil.copy2(source_cover, master / f"MASTER_COVER{source_cover.suffix.lower()}")
...
def create_preview_sheet(project, output_dir, master_pdf, settings):
    """Create a simple marketplace preview sheet from available artwork/cover assets."""
    output_dir.mkdir(parents=True, exist_ok=True)
    preview = output_dir / "PREVIEW_SHEET.png"
    cover = None
    master = project / "MASTER"
    cover_candidates = sorted(master.glob("MASTER_COVER.*")) if master.exists() else []
    if cover_candidates:
        cover = cover_candidates[0]
    if cover is None and (project / "COVER").exists():
        candidates = sorted((project / "COVER").glob("*.png")) + sorted((project / "COVER").glob("*.jpg"))
    ...
    if cover and cover.suffix.lower() in IMAGE_EXTENSIONS:
        try:
            ...
            pass
    try:
        artworks = get_images(project)
        for item in artworks[:5]:
            try:
    ...
    if not images:
        # Still produce a valid preview asset even when the source is a PDF-only import.
        img = Image.new("RGB", (1200, 1600), "white")
        draw = ImageDraw.Draw(img)
        title = str(settings.get("title", project.name))
        draw.text((80, 120), title, fill="black")
...
def write_product_description(project, output_dir, settings, platform_name):
    title = settings.get("title", project.name)
    author = settings.get("author", "")
    description = settings.get("description") or settings.get("cover_blurb") or f"A coloring book by {author}."
    keywords = settings.get("keywords", [])
...
def generate_platform_package(project, platform_name, source_pdf=None, quiet=False):
    ...
    if not profile:
        return {"status": "ERROR", "errors": [f"Unknown platform: {platform_name}"]}

    settings = load_project_settings_safe(project)
    master_pdf, errors, warnings = ensure_master_book(project, source_pdf=source_pdf)
    if errors or master_pdf is None:
    ...
    # Every platform gets a copy from MASTER; nothing edits MASTER.
    digital_pdf = root / f"{title}_{profile.get('output_dir', 'DIGITAL')}.pdf"
    shutil.copy2(master_pdf, digital_pdf)
    generated.append(digital_pdf)

    cover = None
    for candidate in sorted((project / "MASTER").glob("MASTER_COVER.*")):
        cover = candidate
        break
    if cover and cover.suffix.lower() in IMAGE_EXTENSIONS:
        target = root / f"{title}_COVER{cover.suffix.lower()}"
        shutil.copy2(cover, target)
        generated.append(target)

    preview = create_preview_sheet(project, root, master_pdf, settings)
    ...
    manifest = {
        ...
        "title": settings.get("title", project.name),
        "generated": datetime.now().isoformat(timespec="seconds"),
        "files": [str(p.relative_to(project)) for p in generated if p.exists()],
        "warnings": warnings,
    }
    save_json(root / "PACKAGE_MANIFEST.json", manifest)

    if not quiet:
        print(f"\n{platform_name} package READY")
        print(f"Output: {root}")
        print(f"Files: {len(manifest['files'])}")
    return {"status": "READY", "output": root, "manifest": manifest, "warnings": warnings}
...
def import_existing_pdf_to_master(project, pdf_path):
    ...
    if not settings_path.exists():
        save_json(settings_path, {
            ...
            "trim_height": 11,
            "dpi": 300,
            "production_profile": "Imported PDF",
            "factory_version": FACTORY_VERSION,
            "platform_engine_version": PLATFORM_ENGINE_VERSION,
        })
    master, errors, warnings = ensure_master_book(project, source_pdf=Path(pdf_path))
    if errors:
        print("\n".join(f"ERROR: {x}" for x in errors))
        return None
    print(f"\nMASTER PDF created: {master}")
    for warning in warnings:
        print(f"WARNING: {warning}")
    return master
...
def platform_center():
    while True:
        ...
        if choice in {"1", "2", "3"}:
            ...
            project = PROJECTS / unique_project_name(name)
            project.mkdir(parents=True, exist_ok=True)
            ...
            for i, (name, profile) in enumerate(profiles.items(), 1):
                print(f"{i}. {name}: {'ENABLED' if profile.get('enabled') else 'disabled'} | {profile.get('type')}")
            ...
            if raw.isdigit():
                ...
                if 0 <= index < len(names):
                    name = names[index]
                    profiles[name]["enabled"] = not profiles[name].get("enabled", False)
            ...
            print("\nPLATFORM AUDIT")
            print("-" * 60)
            print(f"Master: {'OK' if master else 'MISSING'}")
            print(f"Pages: {pdf_page_count(master) if master else 'unknown'}")
            for error in errors:
                print(f"ERROR: {error}")
            for warning in warnings:
                print(f"WARNING: {warning}")
            for name, profile in profiles.items():
                if not profile.get("enabled"):
                    continue
                out = project / PLATFORM_DIRNAME / profile.get("output_dir", name.upper())
                manifest = out / "PACKAGE_MANIFEST.json"
                print(f"{name}: {'BUILT' if manifest.exists() else 'NOT BUILT'}")
            input("\nPress Enter to continue...")
